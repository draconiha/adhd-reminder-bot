...
# Link newly generated task instances to their recurring template.
...
# Helper for legacy generated instances. Only future, unfinished tasks are linked.
    ...
    try:
        ...
        for recurring in recurring_tasks:
            ...
            try:
                ...
                end_date = None
            cursor.execute(
                "SELECT id, date FROM tasks WHERE user_id=? AND recurring_id IS NULL AND task=? AND reminder_time=? AND remind_before=? AND is_done=0 AND date>=? AND date<=? AND created_at>=?",
                (uid, task_text, reminder_time, remind_before, today.strftime('%Y-%m-%d'), horizon.strftime('%Y-%m-%d'), created_at)
            )
...
'''
    ...
    text = text.replace('callback_data=f"delete_recurring_{task_id}"', 'callback_data=f"delete_recurring_ask_{task_id}"', 1)
...
replacement = '''    elif data.startswith('recurring_view_'):
        ...
        tasks = get_recurring_tasks(user_id)
        result_text = f"✅ Повтор удалён.\\nНевыполненных экземпляров удалено: {deleted_instances}." if deleted_ok else "❌ Повторяющееся дело не найдено."
        if tasks:
...
replacement = '''    elif data == 'recurring_delete_all_confirm':
        ...
        bot.edit_message_text(
            f"✅ Удалено повторов: {deleted_recurring}\\nНевыполненных экземпляров удалено: {deleted_instances}.",
            user_id,
            msg_id,
