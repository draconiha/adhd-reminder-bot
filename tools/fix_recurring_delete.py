...
# 2) New generated instances keep the id of their recurring template.
...
if old_insert in text:
    ...
    raise SystemExit('recurring INSERT marker not found')

# 3) Link already-generated, unfinished instances to their recurring template.
if 'def migrate_legacy_recurring_instances():' not in text:
    helper = '''

def migrate_legacy_recurring_instances():
    """Связывает старые экземпляры повторов с их шаблонами."""
    ...
    try:
        ...
        first_date = today - datetime.timedelta(days=60)
...
# 4) Delete one recurring template and all unfinished generated instances.
pattern = re.compile(
    r"def delete_recurring_task\(task_id, user_id\):\n.*?return True\n",
    re.S,
)
...
# 5) Delete all recurring templates and unfinished generated instances.
pattern = re.compile(
    r"def delete_all_recurring_tasks\(user_id\):\n.*?return deleted_count\n",
    re.S,
)
...
# 6) Make the detail card delete button ask for confirmation.
text = text.replace(
...
# 7) Add the detail-card keyboard if it is missing.
...
'''
    ...
    if marker not in text:
        raise SystemExit('recurring confirmation keyboard marker not found')
    text = text.replace(marker, helper + '\n' + marker, 1)

# 8) Add confirmation keyboard if it is missing.
...
# 9) Add the detail renderer to bot.py if it is missing.
...
# 10) Replace individual recurring deletion callback handling.
handler_pattern = re.compile(
    r"    elif data\.startswith\('recurring_view_'\):.*?    elif data == 'recurring_delete_all_ask':",
    re.S,
)
handler_replacement = '''    elif data.startswith('recurring_view_'):
...
# 11) Update delete-all callback to match the new return value.
...
print('Recurring deletion patch complete')
